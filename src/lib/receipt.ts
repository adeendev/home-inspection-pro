...
export function receiptVerificationCode(
  orderId: string,
...
export async function generateReceiptPdf(order: ReceiptOrder): Promise<Uint8Array> {
  if (!order.stripePaymentIntentId) {
    throw new Error("Cannot generate receipt: order has no payment intent");
  }
  ...
  const code = receiptVerificationCode(order.id, order.stripePaymentIntentId, order.amountCents);
